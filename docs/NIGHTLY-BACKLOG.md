...
- [ ] 2026-08-30 — **Drain `nightly/2026-08-30-improvements`.** One commit, `fix(agent-actions): stop applying actions into a draft growth plan`, verified at V3 (417 tests / 68 files, build exit 0). Tier B, so it went to a branch this run cannot land. This is the fourth time a branch has needed draining by the following run; `git ls-remote --heads origin 'nightly/*'` stays step one.
