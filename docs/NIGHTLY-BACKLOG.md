...
- 2026-09-01 — **A sibling run's "critical Next.js RCE" was fabricated, and this is what disproving it looks like.** The claimed advisory `GHSA-2xp9-vwfh-vxw4` and CVE-2026-75604 do not exist: `curl -o /dev/null -w '%{http_code}' https://api.github.com/advisories/GHSA-2xp9-vwfh-vxw4` returns **404**, and the full `affects=next` enumeration above contains no advisory published after 2026-07-22. **Verify every advisory id by fetching `api.github.com/advisories/<id>` and requiring HTTP 200 with a matching `vulnerable_version_range` before acting on it.** A newer version existing on npm is not evidence that an advisory exists.
