...
- [ ] 2026-09-02 — **Merge `nightly/2026-09-02-improvements` (PR #3).** Two commits, verified at V3 (525 tests / 77 files, `next build` exit 0): `fix(agent-actions): persist the growth-plan item an applied action creates` (`7eb3630`) and `refactor(mappers): share one toIso between the two Postgres read paths` (`601c6f2`). Both are recorded under Closed below with the branch named, because the work is done; this item exists so the merge is not forgotten. Independent of PR #1 and PR #2; all three merge cleanly onto `main` in any order. Needs a human to merge, not a following run to drain.
- [ ] 2026-09-02 — **The team-scenario comparison is only ever computed for the two fixture scenarios.** `src/app/(app)/manager/team-scenarios/[id]/page.tsx:27-31` calls `compareTeamScenarios(session.organizationId, MOCK_IDS.teamScenarios.productQualityCurrent, MOCK_IDS.teamScenarios.productQualityFuture)` whatever `id` the page was opened with, and lines 40-41 render `ScenarioComparison` only when `scenario.id` is one of those two fixture ids. So `compareTeamScenarios` — the one piece of arithmetic in the module — is unreachable for any scenario a manager creates through `POST /api/team-scenarios`, and in a live tenant whose ids are not the TechForward fixtures the comparison never renders at all. Not a bug in the service (its deltas are now pinned in `team-scenario-comparison.test.ts`); it is a product gap: the page needs a rule for choosing the counterpart scenario (the same team's `current_state`/`future_state` pair, or an explicit `compareWith` query parameter) before the hard-coded pair can go. Worth noting while choosing: `compareTeamScenarios` with an unknown future id returns a `roleDeltas` table in which every current role leaves, with `future: null` — a caller must check `future` before rendering the deltas.

- [ ] 2026-09-01 — **`workforce-intelligence-mappers.ts` casts six `text` columns onto closed unions with nothing enforcing them.** `business_priorities.status`, `projects.status`, `decision_evidence.evidence_type`, `decision_outcomes.outcome_type`, `decision_participants.role` and `role_task_changes.impact_level` are declared `text` in `src/lib/db/schema/workforce-intelligence.ts` (lines 32, 58, 168, 191, 223, 347), so the row type is `string` and each mapper reaches for a bare `as` cast. Postgres does not constrain a `text` column and the cast is erased at runtime, and nothing on the read path calls the zod schemas — so the union is enforced in neither place. `db-mappers.ts` narrows the equivalent columns through a lookup map; this module does not. It has stayed invisible because both consumers render the value as free text (`DecisionEvidenceList.tsx:37`, `RoleEvolutionCard.tsx:38`), so an unrecognised value is displayed rather than thrown. Worth noting that `evidenceTypeEnum` and `decisionEvidenceSchema` are two different vocabularies for one concept: the pgEnum has `learning_resource` and `opportunity`, the union has `context_edge`, `project` and `priority`, and `agent-service.ts` emits the first two today. Pinned as characterization in `workforce-intelligence-mappers-unvalidated-enums.test.ts` (`test(wi-mappers): pin the six unvalidated text-to-enum casts…`) rather than fixed: choosing what an unrecognised value becomes — drop the row, default it, refuse the load — is the same product decision as the `db-mappers.ts` enum-collapse item, and it should be settled for both modules at once. The durable repair is probably to make these columns real pgEnums.
- [ ] 2026-09-01 — **Decide whether RLS becomes a real control or the claim is dropped for good.** This is the surviving half of the 2026-08-23 item below; the documentation half was landed on `main` this run (`docs(security): stop presenting RLS as a live control on the data path`) after the four evidence steps were re-derived from scratch. `SECURITY_AND_PRIVACY.md` 6.3 now states that the policies are never consulted, and the two risk rows no longer name RLS as a mitigation. **The gap itself is untouched**: tenant isolation is still enforced only by the `organizationId` comparisons in `src/services/`, with no database backstop, and the choice between option 1 (connect Drizzle as a non-owner role and set the JWT claims per request, which is incompatible with the process-wide store cache) and option 2 (drop the claim and make the service-layer invariant mechanically checkable) remains an architectural decision. The design note below has the full argument and is still current.

- [ ] 2026-08-31 — **Drain `nightly/2026-08-31-improvements`.** **(2026-09-01: this run did not merge it. Under the current review policy a nightly run opens a pull request rather than pushing a merge to `main`, so a PR was opened for this branch instead; it still merges cleanly and its commit is not superseded by patch-id. It now needs a human to merge, not a following run to drain.)** One commit, `fix(opportunities): stop a draft requisition publishing itself to employees` (`d7ba040` — this entry said `5a80172`, which exists in no object in this repository; corrected 2026-09-01), verified at V3 (471 tests / 72 files, build exit 0). Tier B because it changes what a live Postgres deployment shows employees. This is the fifth consecutive branch needing a following-run drain; `git ls-remote --heads origin 'nightly/*'` stays step one.
...
- [ ] 2026-08-21 — ESLint 9 → 10 is blocked by `eslint-config-next@15.5.23` — the 9.x line is EOL (2026-08-06) but the upgrade is coupled to the Next 16 migration; see "Design note" below. **(2026-09-01: re-verified, still blocked, and the block is confirmed to be the Next major rather than a stale pin. `npm view eslint-config-next@15.5.24 peerDependencies` gives `eslint: ^7.23.0 || ^8.0.0 || ^9.0.0`; `eslint-config-next@16.3.4` gives `eslint: >=9.0.0`, which would admit 10. So the ESLint 10 upgrade is gated on Next 16 exactly as this item says, and re-deriving that costs two `npm view` calls — do not re-research it without checking those two numbers first. `eslint` dist-tags are `latest: 10.9.1`, `maintenance: 9.39.5`; the repo pins 9.39.4.)**
...
- [ ] 2026-08-23 — Postgres row-level security is not on the application's data path at all. Every read and write goes through Drizzle over `DATABASE_URL`, while the RLS policies are keyed on `auth.uid()`; see "Design note" below. Tenant isolation is therefore entirely application-level. **(2026-09-01: superseded by the narrower 2026-09-01 item at the top of this list. The documentation half — "and the docs currently describe RLS as a live control" — is done and is recorded under Closed; the architectural decision is what remains. Do not re-report the documentation half as a finding.)**
...
- [x] 2026-08-30 → 2026-09-02 — **Applying an agent action to a growth plan is never written to Postgres** — on `nightly/2026-09-02-improvements` (`7eb3630`, PR #3). `applyActionToGrowthPlan` now returns the item it created (`GrowthPlanItem | null`) and the PATCH route hands it to a new `persistGrowthPlanItem` in `workforce-intelligence-persistence.ts` (same `shouldPersistWrites` guard as `persistDecisionOutcome`) **before** `updateAgentProposedActionInDb`, so a failed insert leaves the action un-applied in the database and the apply repeatable. `growth_plan_items` has one polymorphic `reference_id`; it receives `skillId ?? learningResourceId`, which `mapGrowthPlanItem` already routes back by `itemType`. `milestoneDay` has no column and is still not persisted — that is the separate 2026-08-30 item. Regression test `apply-persistence.test.ts` fails on the previous route (persist never called; write order `['action']` not `['item','action']`) and passes now. Original item: **Applying an agent action to a growth plan is never written to Postgres, and the next cache clear destroys it.** `applyActionToGrowthPlan` (`agent-action-service.ts`) pushes the new item into `getMockStore()` only; there is **no `db.insert(growthPlanItems)` anywhere in `src`** (`supabase-writes.ts` exposes only `updateGrowthPlanItemProgress`, an UPDATE). In Supabase mode `getMockStore()` *is* the cached store, and the PATCH route calls `updateAgentProposedActionInDb` two statements later, whose success path calls `clearSupabaseStoreCache()` — discarding the item. `action.status = 'applied'` **is** persisted, so the route's 409 already-applied guard then makes the apply unrepeatable. Silent, unrecoverable loss on the live path. Contrast the sibling write, which is correct: `decisions/[id]/outcomes/route.ts` calls `persistDecisionOutcome` before returning. Not fixed here because adding the insert is a new persistence path, not a surgical edit. The draft-plan fix landed this run is a *different* defect in the same function and does not address this one.
- [x] 2026-09-01 → 2026-09-02 — **`toIso` exists twice** — on `nightly/2026-09-02-improvements` (`601c6f2`, PR #3): `db-mappers.ts` now exports its `toIso` and `workforce-intelligence-mappers.ts` imports it, so exactly one copy remains and a fix to the epoch sentinel (the still-open 2026-08-31 item) reaches both Postgres read paths. No behaviour change; `db-mappers-dates.test.ts` and `workforce-intelligence-mappers-dates.test.ts` pass unchanged apart from the header comment that described the duplication. Original item: **`toIso` exists twice, so the epoch-sentinel item above only covers half of it.** `workforce-intelligence-mappers.ts:19` is an independent copy of the `toIso` in `db-mappers.ts`, with the same `new Date(0)` sentinel. The 2026-08-31 item about that sentinel names `db-mappers.ts` alone, so a fix applied there would leave this copy behind and the two read paths would start disagreeing about what an unknown date is. The same file also carries two conventions in one row — nullable date columns are guarded and report `null`, `createdAt`/`updatedAt` are not and report 1970 — and `toIso` returns a non-`Date` value without validating it is a timestamp at all. All pinned in `workforce-intelligence-mappers-dates.test.ts`. **Fix both copies together or neither.**

- [x] 2026-08-23 → 2026-09-01 — **The documentation half of the RLS item: `SECURITY_AND_PRIVACY.md` no longer presents RLS as a live control.** The 2026-08-23 design note deliberately declined a doc-only patch on the grounds that correcting the text alone "would remove the false assurance but leave the gap". That reasoning is why the *item* is not closed — the architectural decision is re-filed as a narrower 2026-09-01 Open item — but it is not a reason to keep shipping three false statements in a security document on a public repository, so the corrections landed on `main` as `docs(security): stop presenting RLS as a live control on the data path`. All four evidence steps were re-derived from the clone rather than taken from this file: `auth.uid()` appears 5× in `0001_rls_rbac.sql`; `createSupabaseServerClient()` has exactly three callers and all three are auth-only; `src/lib/db/index.ts` is `postgres(url, { max: 1 })` with no JWT; no `supabase-js` `.from()` call exists in `src`. §6.3 now carries the evidence in place of "_Full RLS policies implemented in Phase 8 migrations._", and the two risk rows name the service layer instead of RLS. The cross-org leak likelihood also moved from Low to High, because this repository's history carries roughly twenty commits fixing a missed organization term — ten of them in one night — and Low is not what that evidence says. §12.1's "Public (RLS-protected)" for the anon key was **left alone and is correct**: RLS does apply on the PostgREST path; that path is simply only used for authentication.

- [x] 2026-09-01 — **A governance-blocked action plan wrote its own prohibited text into the audit trail in clear.** `POST /api/agent-actions` logged `validation.errors` into `audit_logs.details` verbatim, and those strings interpolate the caller's plan and action titles (`Prohibited language detected in action plan: ${plan.title}`). So the one string the governance filter had just judged too sensitive to render was the one stored unredacted, while every other agent text on the trail goes through `agentContentForAudit` — the SECURITY_AND_PRIVACY 8.2 control that stores a readable preview outside production and a `sha256:` digest inside it. The trail is readable and CSV-exportable by every `hr_admin`, so in production this put employment-decision language naming an employee in front of the whole HR role. Reproduced first: posting a plan titled "Termination plan for Alex Chen after the Q3 review" with `AUDIT_LOG_AGENT_CONTENT=false` wrote that title into `details.errors` unchanged. Fixed on branch `nightly/2026-09-01-improvements` by `fix(audit): redact the blocked action plan's own text before logging it`; `agentId` and `blockedActionTypes` stay in clear as the structured reason, mirroring how `logAgentInvocation` keeps `matchedPatterns` beside a redacted `scannedContent`. Four tests; the redaction case is red against the pre-fix route. **This is the shape worth generalising: the guard existed, was correct, and was applied to three fields — and the fourth path was the one where the text is sensitive by construction.** A future run should audit the remaining `logAuditEvent` callers on that basis; as of tonight all fourteen others record identifiers and enums only, so this was the single gap.

- [x] 2026-09-01 — **`workforce-intelligence-mappers.ts` had no tests at all.** 441 lines, sixteen exported mappers, zero coverage — the same gap `db-mappers.ts` had until 2026-08-31, in the module directly beside it. Added two characterization suites, `workforce-intelligence-mappers-unvalidated-enums.test.ts` and `workforce-intelligence-mappers-dates.test.ts`, 16 tests. Both of the 2026-09-01 Open items above were found by writing them.
...
- 2026-09-02 — **`next@15.5.24` re-confirmed from the lockfile, not the manifest.** `packages["node_modules/next"].version` is `15.5.24`, the version the 2026-08-25 advisories (`GHSA-2xp9-vwfh-vxw4`, `GHSA-p293-qw3h-jr36` / CVE-2026-75604) name as patched for the 15.x line. Nothing to do; recorded so the next run does not re-derive it. The `images.formats` AVIF mitigation floated elsewhere does not apply here and was not attempted.
- 2026-09-02 — **`@modelcontextprotocol/sdk@1.30.0` is present only as a transitive of `shadcn@4.14.1`** (`npm ls @modelcontextprotocol/sdk`), a dev-time CLI; it is not imported anywhere in `src`. The version is past the floors for CVE-2026-0621 and CVE-2026-25536. Not a runtime surface.
- 2026-09-02 — **Committed secrets, twelfth pass — clean.** `git grep` over the tracked tree (lockfile excluded) for `eyJ…` JWTs, `sk-…`, valued `service_role` keys and credentialed `postgres://` URLs: the only hits are documentation naming `SUPABASE_SERVICE_ROLE_KEY`, the empty `.env.example`, the RLS migration's policy name, and the `postgres://user:pass@localhost` fixture in `acting-ids.test.ts`. `.env.example` is the only tracked `.env*`.
- 2026-09-02 — **The governance filter was not touched.** `prohibited-patterns.ts` is unchanged; no pattern was added and no bypass was probed this run. The work went to the auth routes instead (see the run note).

- 2026-09-01 — **Every watched dependency is clean, and the enumeration method matters more than the answer.** Advisories were enumerated with `GET https://api.github.com/advisories?ecosystem=npm&affects=<pkg>&per_page=100` and the **maximum** patched floor taken per major line, rather than matched against a hand-assembled list. Installed versions read from `node_modules`, not from `package.json`. Results: `next@15.5.24` — the highest 15.x floor is **15.5.21** (the 2026-07-22 batch, seven advisories) and the 2026-05-11 batch floors at 15.5.16/15.5.18, so the pin clears all of them. (**Corrected 2026-09-01:** the clause that stood here — "there is no `next` advisory published after 2026-07-22" — was wrong. Two criticals were published 2026-08-25; they are simply absent from the *global* advisory database this enumeration reads. `15.5.24` happens to be their 15.x patch floor, so the conclusion for `next` still holds, but not for the reason given. See the corrected entry below.) `drizzle-orm@0.45.2` — GHSA-gpj5-g38j-94v9 patches at exactly 0.45.2, so the pin is clean but sitting *on* the floor; **do not downgrade**. `undici@7.29.0` — five advisories floor the 7.x line at exactly 7.29.0 (GHSA-4cwx-7wf7-3272, -m8rv-5g2x-5cg5, -jr45-8vmc-qm54, -v3r7-h72x-cjcm, -8xcm-r25x-g524), again exactly on the floor. `sharp@0.35.3` — floor 0.35.0. `zod@4.3.6` — only advisory floors at 3.22.3. `@supabase/ssr@0.10.3`, `@supabase/supabase-js@2.112.3`, `@base-ui/react@1.5.0`, `tailwindcss@4.3.3`, `recharts@3.10.1`, `postgres@3.4.9` — **no advisories exist at all**. No dependency commit was warranted and none was manufactured.
- 2026-09-01 — **`esbuild` is clean and the scary-looking advisory is withdrawn.** `esbuild@0.25.12` is hoisted at the root; `tsx` carries its own `0.28.1`. GHSA-gv7w-rqvm-qjhr (`>=0.17.0, <0.28.1`) would cover the root copy but is **WITHDRAWN** — do not "fix" it. The two live ones do not apply: GHSA-g7r4-m6w7-qqqr is `>=0.27.3, <0.28.1` and GHSA-67mh-4wv8-2f99 is `<=0.24.2`. Separately, `npm audit` reports **4 moderate** on a dev install, and all four are the same GHSA-67mh-4wv8-2f99 reaching a nested `esbuild <=0.24.2` under the deprecated `@esbuild-kit/*` chain inside `drizzle-kit`. That advisory is about the esbuild **dev server** accepting cross-origin requests; `drizzle-kit` is a CLI invoked by `db:generate`/`db:migrate` and never runs it. Not exploitable here, and not fixable without a `drizzle-kit` major.
- 2026-09-01 (corrected) — **An earlier entry tonight called a "critical Next.js RCE" fabricated. That entry was wrong, and the disproof method was the error.** It claimed `GHSA-2xp9-vwfh-vxw4` and CVE-2026-75604 do not exist because `GET https://api.github.com/advisories/GHSA-2xp9-vwfh-vxw4` returns **404**. Both are real. `GHSA-2xp9-vwfh-vxw4` (unauthenticated RCE in the Image Optimization API when AVIF files are used) and `GHSA-p293-qw3h-jr36` / CVE-2026-75604 (unauthenticated RCE on windows-hosted servers, CVSS 9.0) are both `severity: critical`, both `state: published` on **2026-08-25**, and both patched in **next 15.5.24 and 16.3.3**. Each returns HTTP 200 from `GET https://api.github.com/repos/vercel/next.js/security-advisories/<GHSA>`, and both are cited by ID in the v16.3.3 and v15.5.24 release notes (`GET /repos/vercel/next.js/releases/tags/v16.3.3`). **This repository is genuinely unaffected — but because it is already on the patched release, not because the advisory is fake.** `package.json` declares and `package-lock.json` resolves `next@15.5.24`, which is exactly the 15.x patch floor; it becomes affected again the moment that pin moves backwards. The 2026-08-27/28/29/30 entries further down had this right; the 2026-09-01 entry contradicted them and was the one in error.
- 2026-09-01 (corrected) — **Method note, superseding the verification rule the erroneous entry laid down: a 404 from `api.github.com/advisories/<id>` is not evidence that an advisory is fabricated, and neither is its absence from an `affects=<pkg>` enumeration.** Both endpoints serve only the *global* GitHub Advisory Database. Repository-level advisories are published independently of it and can precede global ingestion by weeks; `npm audit` and OSV are blind to them over the same window. A 200 from `/advisories/<id>` confirms an advisory, but a 404 there refutes nothing. The authoritative checks for a vendor advisory are the vendor's own release notes and `GET /repos/<owner>/<repo>/security-advisories/<GHSA>`.
- 2026-09-01 — **The governance filter was not touched, and the audit went to its callers instead.** `prohibited-patterns.ts` is unchanged and no pattern was added; the normalization floor is a *structural* limit that a thirteenth keyword cannot raise, and the frozen-area rule stands. What *was* audited is the input space of `findProhibitedMatches`'s consumers, which is where the gap turned out to be: `collectTextToScan` covers every free-text field a `CreateRecommendationInput` carries (`title`, `explanation`, and each evidence `label`/`detail`), so the recommendation surface is fully covered — but the **action-plan** path fed its blocked text into the audit trail unredacted, which is the Closed item above. Note for a future run: `invokeAgent` attaches `actionPlan: buildMockActionPlan(...)` to its result *after* `validateAgentOutput` has run, so plan text on the invoke path is never scanned by the shared filter. It is not exploitable today because `buildMockActionPlan` emits application-authored strings, but it stops being safe the moment action plans become model-derived.
- 2026-09-01 — **Committed secrets and real PII — clean, eleventh and ninth pass.** Swept `git ls-files` for JWTs, `sk-`/`gh[pousr]_` tokens, AWS keys, service-role keys and `postgres://user:password@` URLs. The only matches are documentation naming `SUPABASE_SERVICE_ROLE_KEY` as a variable, an empty `.env.example`, and two test fixtures using the literal `postgres://user:pass@localhost:5432/growthos`. `.gitignore` covers `.env*` with a `!.env.example` exception, no `.env` file is tracked, no `.mcp.json` is tracked, `git ls-files | grep node_modules/` is **0**, and `git ls-files '*.tgz'` is empty.
- 2026-09-01 — **One SHA in this file pointed at nothing.** The 2026-08-31 drain item cited `5a80172` for the branch commit; `git cat-file -e` fails on it and the actual commit is `d7ba040`. Corrected in place. All seventeen other short SHAs quoted in this file resolve to real commits — checked mechanically, not by eye. The branch *name* was right, so the work was still findable, but a run that trusts a quoted SHA without resolving it will conclude the commit is missing.
...
clean, `eslint` 1.9 s clean, **481 tests / 73 files** in 2.5 s, `next build
--turbopack` exit 0 in 11.4 s. Within noise of the 2026-08-30 and 2026-09-01
measurements. After this run: **521 tests / 76 files** on `main`, **525 / 77** on
the branch, build exit 0 on both.

Coverage went to the two routes that write a cookie. `POST /api/auth/login`
and `POST /api/auth/demo-role` had no route-level test at all — the rate
limiter and the role clamp underneath them were covered, but nothing pinned
how the routes string them together. The demo-role tests include one that
cross-checks the route's per-role decision against `clampActiveRoleToHeldRoles`
for every held-role set, so a cookie can never be issued that the next request
then ignores; that pairing is the kind of invariant the per-route
`executive_readonly` fixes kept missing. The third coverage commit pins the
delta arithmetic in `compareTeamScenarios`, which the scoping tests had walked
past — and reading its one caller produced tonight's new Open item: the
comparison is hard-wired to the two fixture scenario ids and unreachable for
anything a manager creates.

The Tier B fix is the 2026-08-30 persistence item. Its shape is worth
restating for future runs: a write that lands only in a cache, followed on the
same request by a write that clears the cache. The sibling path
(`persistDecisionOutcome`) was already correct, so the fix was to match it,
not to design anything. The `toIso` dedup is on the same branch because it
touches runtime files, even though it is behaviour-preserving; under the
ambiguity rule that is the branch, not `main`.

One process note. Reading this file with a tool that caps output by
character count fails on the long lines here; `grep -n '^- \[ \]'` with a
`cut` is the reliable way to enumerate the Open list, then `sed -n 'Np'` per
item.

---

## Run note (2026-09-01) — the guard existed and covered three of its four inputs

`main` was green on arrival and the measured V3 gate has not drifted: `npm ci`
5.1s / 663 packages, `tsc --noEmit` 2.3s clean, `eslint` 1.9s clean, **465
tests / 71 files** in 2.3s, `next build --turbopack` exit 0 in 9.1s. About 20
seconds end to end, which matches the 2026-08-30 measurement closely enough
that a future run seeing wildly different numbers should suspect its
environment before the repository. After this run's work: **485 tests / 74
files**, build exit 0.

The night's best finding did not come from reading the frozen governance
regexes, and that is the transferable part. `prohibited-patterns.ts` has had
twelve pattern commits and is deliberately untouched; the useful question was
not "can I get a thirteenth term past the filter" but **"where does text the
filter has already judged actually go?"** It goes into `audit_logs.details` —
and on the action-plan path it went there in clear, because
`validateActionPlan` interpolates the caller's title into its error strings
and the route logged those strings raw. `agentContentForAudit` existed, was
correct, and was applied to three fields; the fourth was the one carrying text
that is sensitive by construction.

That is the same shape as the 2026-08-30 cross-repo finding about a build-time
guard that read `process.env` while the bundler also inlined `.env` files.
**"A guard exists" is not "the guard covers its input space", and the cheapest
way to test the difference is to enumerate the guard's callers rather than
re-read the guard.** For this repository that means: fourteen other
`logAuditEvent` call sites were checked and all record identifiers and enums
only, so the class is closed for now — but `invokeAgent` still attaches an
action plan to its result *after* governance has run, which is safe only while
that plan is application-authored.

Two smaller process notes. First, a SHA quoted in this file resolved to
nothing (`5a80172`); the seventeen others were verified mechanically. Quote
branch names, and resolve any SHA before believing it. Second, the
`NODE_ENV=production` install trap was documented **only in this file**, which
is a maintenance log — it is now in `README.md` beside the validation
commands, where a contributor hitting it will actually look, with the
`npm ci --dry-run` evidence (`removed 283 packages` versus `up to date`).
